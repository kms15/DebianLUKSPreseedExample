...
# include the public temporary ssh key in the tftp directory
tftp/authorized_keys: id_iso_root_rsa.pub
	cp $< $@

TFTP_COMPLETE_TAG : tftp/debian-installer/amd64/initrd0.gz tftp/pxelinux.cfg/default \
		tftp/authorized_keys
	touch $@
...
qemu-pxe-test : qemu-pxe-test-image.qcow2 TFTP_COMPLETE_TAG
	qemu-system-x86_64 -hda $< -m 512M -smp 1 -accel kvm -boot cn \
		-nic user,hostfwd=tcp:127.0.0.1:10022-:22,hostfwd=tcp:127.0.0.1:10023-:23,tftp=./tftp,bootfile=/pxelinux.0 \
		-curses # -boot d
